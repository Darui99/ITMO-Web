...
public class FrontServlet extends HttpServlet {
    ...
    private Method getMethod(Class<?> pageClass, String name) {
        ...
        for (Method method : pageClass.getSuperclass().getDeclaredMethods()) {
            if (name.equals(method.getName())) {
                return method;
            }
        }
    ...
    private void process(Route route,
        ...
        Method before = null, method = null, after = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                before = getMethod(pageClass, "before");
                method = getMethod(pageClass, route.getAction());
                after = getMethod(pageClass, "after");
        ...
        try {
            ...
            for (Class<?> elem : method.getParameterTypes()) {
                if ("Map".equals(elem.getSimpleName())) {
                    params.add(view);
                }
                if ("HttpServletRequest".equals(elem.getSimpleName())) {
                    params.add(request);
                }
            }
