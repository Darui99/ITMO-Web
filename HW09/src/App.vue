    ...
    export default {
        ...
        components: {
            ...
            this.$root.$on("onRegister", (login, userName) => {
                let letters = /^[a-z]+$/;
                if (!login || login.length < 3 || login.length > 16 || !letters.test(login)) {
                    this.$root.$emit("onRegisterValidateError", "Login is invalid");
                    return;
                }
                if (!userName || userName.length === 0 || userName.length > 32) {
                    this.$root.$emit("onRegisterValidateError", "User's name is invalid");
                    return;
                }

                let users = Object.values(this.users).filter(u => u.login === login);
                if (users.length > 0) {
                    this.$root.$emit("onRegisterValidateError", "Such login is already used");
                    return;
                }
